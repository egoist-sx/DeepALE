...
//  Copyright (c) 2014 Shuai Zheng. All rights reserved.
//
#include "kyheader.h"
#include "FilterTIG.h"
...
void FilterTIG::reconstruct(cv::Mat &w1f){
    w1f = cv::Mat::zeros(8, 8, CV_32F);
...
cv::Mat FilterTIG::matchTemplate(const cv::Mat &mag1u){
    const int H = mag1u.rows, W = mag1u.cols;
    const cv::Size sz(W+1, H+1); // Expand original size to avoid dealing with boundary conditions
    cv::Mat_<INT64> Tig1 = cv::Mat_<INT64>::zeros(sz), Tig2 = cv::Mat_<INT64>::zeros(sz);
    cv::Mat_<INT64> Tig4 = cv::Mat_<INT64>::zeros(sz), Tig8 = cv::Mat_<INT64>::zeros(sz);
    cv::Mat_<byte> Row1 = cv::Mat_<byte>::zeros(sz), Row2 = cv::Mat_<byte>::zeros(sz);
    cv::Mat_<byte> Row4 = cv::Mat_<byte>::zeros(sz), Row8 = cv::Mat_<byte>::zeros(sz);
    cv::Mat_<float> scores(sz);
    for(int y = 1; y <= H; y++){
    ...
    cv::Mat matchCost1f;
    scores(cv::Rect(8, 8, W-7, H-7)).copyTo(matchCost1f);
    return matchCost1f;
}
